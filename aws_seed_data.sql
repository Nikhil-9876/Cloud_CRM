-- ============================================================
-- AWS SEED DATA — run this against your RDS PostgreSQL database
-- AFTER running aws-schema.sql
--
-- BEFORE RUNNING:
--   Replace 'YOUR_COGNITO_SUB_HERE' with your actual Cognito user sub.
--   You can find it in:
--     AWS Console → Cognito → User Pools → your pool → Users → click your user → look for "sub"
--   It looks like: a1b2c3d4-e5f6-7890-abcd-ef1234567890
-- ============================================================

do $$
declare
  u text := 'e4a8b458-a0d1-7092-bf67-6fc7b73c100d';   -- <-- REPLACE THIS
...
begin
  if u = 'YOUR_COGNITO_SUB_HERE' then
    raise exception 'Please replace YOUR_COGNITO_SUB_HERE with your actual Cognito user sub before running.';
  end if;

  -- ----------------------------------------------------------------
  -- CONTACTS
  -- ----------------------------------------------------------------
  insert into contacts (id, user_id, first_name, last_name, email, phone, company_name, notes, created_at) values
  ...
  -- LEADS
  -- ----------------------------------------------------------------
  insert into leads (user_id, name, email, source, status, assigned_to, notes, created_at) values
  ...
  -- DEALS
  -- ----------------------------------------------------------------
  insert into deals (id, user_id, contact_id, title, value, stage, expected_close_date, notes, created_at) values
  ...
  -- ACTIVITIES
  -- ----------------------------------------------------------------
  insert into activities (user_id, contact_id, deal_id, type, title, description, due_date, done, created_at) values
